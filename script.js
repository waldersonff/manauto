// Load products from server (products.json) or localStorage
async function loadProductsData() {
    try {
        ...
        if (stored) {
            ...
            if (Array.isArray(parsed) && parsed.length > 0) {
                console.log(`✓ Carregados ${parsed.length} produtos do localStorage`);
                return parsed;
            }
        }
    } catch (e) {
    ...
    // Se não tem no localStorage, carregar do servidor
    try {
        const response = await fetch('products.json');
        if (response.ok) {
            const data = await response.json();
            if (Array.isArray(data) && data.length > 0) {
                console.log(`✓ Carregados ${data.length} produtos do servidor`);
                return data;
            }
        }
    } catch (e) {
        console.warn('Falha ao carregar products.json do servidor.', e);
    }

    // Se tudo falhar, retorna array vazio
    console.warn('Nenhum produto disponível. Array vazio retornado.');
    return [];
}
...
// Initialize products after loading from server
(async () => {
    products = await loadProductsData();
    filteredProducts = [...products];
    
    // Call init after products are loaded
    init();
})();
