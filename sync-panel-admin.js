...
document.addEventListener('DOMContentLoaded', () => {
    // Apenas carregcar em desenvolvimento
    if (window.AppConfig?.isDevelopment?.()) {
        ...
        console.log('📦 Painel de sincronização desativado em produção');
    }
});

// Atalho teclado: Ctrl+Shift+P para abrir painel (apenas desenvolvimento)
document.addEventListener('keydown', (e) => {
    if (window.AppConfig?.isDevelopment?.() && (e.ctrlKey || e.metaKey) && e.shiftKey && e.key === 'P') {
        e.preventDefault();
        toggleSyncPanel();
    }
});
